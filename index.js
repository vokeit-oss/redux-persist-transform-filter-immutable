import { Collection, Map, isCollection } from "immutable";
...
export function createTransform(dataToStorage, dataFromStorage, config = {}) {
  const whitelist =
    isObject(config) && Array.isArray(config.whitelist)
      ? config.whitelist
      : null;
  const blacklist =
    isObject(config) && Array.isArray(config.blacklist)
      ? config.blacklist
      : null;

  function whitelistBlacklistCheck(key) {
    return (
      (whitelist && !includes(whitelist, key)) ||
      (blacklist && !includes(blacklist, key))
    );
  }

  function transformDataToStorage(state, key) {
    return !whitelistBlacklistCheck(key) && dataToStorage
      ? dataToStorage(state, key)
      : state;
  }

  function transformDataFromStorage(state, key) {
    return !whitelistBlacklistCheck(key) && dataFromStorage
      ? dataFromStorage(state, key)
      : state;
  }
...
export function createFilter(
  ...
  transformType = includes(["whitelist", "blacklist"], transformType)
    ? transformType
    : "whitelist";
  ...
  return createTransform(
    ...
    {
      [transformType]:
        "string" === typeof reducerName ? [reducerName] : reducerName,
    }
  );
}
  ...
  if (!blacklist) {
      ...
      if (isObject(path) && !Array.isArray(path)) {
        if (path.hasOwnProperty("path")) {
          ...
          if (!isEmpty(value)) {
            iterable
              ? (subset = subset.setIn(
                  Array.isArray(path.path) ? path.path : [path.path],
                  value
                ))
        ...
        const value = iterable
          ? state.getIn(Array.isArray(path) ? path : [path])
          : get(state, path);
    ...
    paths.forEach((path) => {
      if (isObject(path) && !Array.isArray(path)) {
            ...
            if (Array.isArray(value)) {
              iterable
                ? (subset = subset.setIn(
                    Array.isArray(path.path) ? path.path : [path.path],
                    subset
                      .getIn(Array.isArray(path.path) ? path.path : [path.path])
                      .filter((x) => false)
                  ))
                : set(
                    subset,
                    path.path,
                    get(subset, path.path).filter((x) => false)
                  );
            } else {
              forIn(value, (value, key) => {
                iterable
                  ? (subset = subset.deleteIn([path.path, key]))
        ...
        if (!isUndefined(value)) {
          iterable
            ? (subset = subset.deleteIn(Array.isArray(path) ? path : [path]))
            : unset(subset, path);
        }
      }
    });
